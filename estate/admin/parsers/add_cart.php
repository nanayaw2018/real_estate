...
// check if the domain is localhost, the return false
...
if($cart_id != '' || $cart_id != 0){
	...
	$previous_items = json_decode($cart['items'], true); //parsing json file in associative array not an object
	$item_match = 0;
	$new_items = array();
	foreach ($previous_items as $pitem) {
		if($item[0]['id'] == $pitem['id'] && $item[0]['size'] == $pitem['size']){
			$pitem['quantity'] = (string) ($pitem['quantity'] + $item[0]['quantity']);
	...
	setcookie(CART_COOKIE, '', 1, "/", $domain, false);//destroy previous cookie
